...
public:
  ...
  void stop();
  void add_task(const thread_pool::task_t* task);  //thread safe;
  void* add_Future_task(const task_t*,void* param,void ** return_value);  //thread safe;
...
private:
  ...
  typedef std::deque<func_wrapper*> tasks_t;//将task和参数作为一个pair
