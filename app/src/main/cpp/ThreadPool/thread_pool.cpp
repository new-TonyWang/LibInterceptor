...
void thread_pool::thread_loop()
{
  ...
  while(m_is_started)
  {
    func_wrapper* task_pair = take();
    if(task_pair->task_t)
    {
     *task_pair->return_value = (*(task_pair->task_t))(task_pair->param);
    }
  }
  __SOLA_LOG(debug, "thread_pool::threadLoop() tid : " + get_tid() + " exit.");
}

    void *thread_pool::add_Future_task(const thread_pool::task_t* task, void *param,void** return_value) {
      ...
      */
      func_wrapper wrapper{task,param,return_value};
      if(){

      }
      m_tasks.push_back(&wrapper);
      m_cond.notify_one();
...
void thread_pool::add_task(const thread_pool::task_t* task)
{
  add_Future_task(task, nullptr, nullptr);
}

thread_pool::func_wrapper* thread_pool::take()
{
  std::unique_lock<std::mutex> lock(m_mutex);
