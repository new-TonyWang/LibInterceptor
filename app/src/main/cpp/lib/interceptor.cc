...
namespace interceptor {
    class InstAndOffset{
    public:
        InstAndOffset()= default;
        InstAndOffset( llvm::MCInst inst,uint64_t offset);
    private:
        llvm::MCInst inst;
    public:
...
class InterceptorImpl {
 ...
 private:
  ...
  Error ReadMemory(void *target,size_t num);
  ...
  Error CheckHookedOrTrampline(void* *old_func,size_t rewrite_size);
...
Error InterceptorImpl::ReadMemory(void *target,size_t num){
    int prot = PROT_READ|PROT_EXEC;
    Error error = ChangePageProtection(target, num, prot);
    if (error.Fail()) return error;
    return error;
}

//Error InterceptorImpl::UnReadMemory(void *target,size_t num){
//    int prot = PROT_READ;
//    Error error = ChangePageProtection(target, num, prot);
//    if (error.Fail()) return error;
//    return error;
...
Error InterceptorImpl::InstallTrampoline(const TrampolineConfig &config,
                                         void *old_function,
                                         void *new_function) {
  size_t initial_alignment = GetCodeAligment(target_.get(), old_function);//获得老函数的地址的布局(小于等于4)
  std::unique_ptr<CodeGenerator> codegen(
      target_->GetCodeGenerator(old_function, initial_alignment));
  if (!codegen) return Error("Failed to create a code generator!");

  Error error =
      target_->EmitTrampoline(config, *codegen, old_function, new_function);//生成跳板（old_function在aarch64中未起作用）
  if (error.Fail()) return error;
  ...
  const llvm::SmallVectorImpl<char> &trampoline = codegen->GetCode();//获得整个trampoline

  std::vector<uint8_t> original_code(trampoline.size());
  memcpy(original_code.data(), load_address, trampoline.size());//获得被跳板覆盖的原函数
...
Error InterceptorImpl::RewriteInstructions(
  ...
  ReadMemory(func_addr, rewrite_size);
  bool reached_end_of_function = false;

  while (offset < rewrite_size) {
    llvm::MCInst inst;
...
Error InterceptorImpl::InterceptFunction(void *old_function, void *new_function,
  ...
  TrampolineConfig full_config = target_->GetFullTrampolineConfig();
  Error error = GetTrampolineSize(full_config, nullptr, new_function,//从中间跳板到目的函数的大小
  ...
  for (const auto &config : configs) {
    if (config.IsFullTrampoline()) {
      ...
      //检测跳板函数和是否被hook，若是跳板函数则会修改old_function
      error = CheckHookedOrTrampline(&old_function,trampoline_size);
      ...
      size_t trampoline_size = 0;
      error = GetTrampolineSize(config, old_function, intermediate_trampoline,//从old_function中间跳板
                                trampoline_size);
      if (error.Fail()) return error;
...
Error InterceptorImpl::CheckHookedOrTrampline(void **old_function,size_t rewrite_size) {
  ...
  while (offset < rewrite_size) {
    ...
    if (!disassembler->GetInstruction(func_addr, offset, inst, inst_size))
      return Error("Failed to disassemble instruction at %p + %zd", func_addr,
                   offset);
    if(target_->CheckHook(inst)){//检测跳转指令如果存在则不进行hook
      return Error("Function has been hooked, skip this Function");
    };
    offset += inst_size;
    //若已经达到了函数结尾但是仍然放不下跳板函数则会从根据B指令跳转的路径(包括多层跳板)进行跳转;
    if(target_->EndOfFunction(inst)&&offset < rewrite_size){//一旦调用到BL或者B指令就认为到了函数的结束
      offset = 0;
      target_->SkipTrampolineFunction(func_addr,offset,inst);//修改func_addr
      target_->ResetState();
      *old_function = func_addr;//重置原函数
    }
  }
    return Error();
}
...
InstAndOffset::InstAndOffset(llvm::MCInst inst, uint64_t offset) {
    this->inst =std::move(inst);
    this->offset = offset;
}

const llvm::MCInst &InstAndOffset::getInst() const {
  return inst;
}

uint64_t InstAndOffset::getOffset() const {
  return offset;
}
