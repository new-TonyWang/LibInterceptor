...
class TargetAARCH64 : public Target {
 public:
  ...
  size_t GetCodeAlignment() const override { return 4; }//按照4的倍数存储
  ...
  void SkipTrampolineFunction(void* &data,size_t offset,
                                 llvm::MCInst &inst) override ;

   bool EndOfFunction(const llvm::MCInst &inst) override;

   bool CheckHook(const llvm::MCInst &inst) override;
 private:
  enum TrampolineType {
