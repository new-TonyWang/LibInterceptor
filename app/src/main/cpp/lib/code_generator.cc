...
CodeGenerator::CodeGenerator(llvm::Triple triple, size_t start_alignment)
        : start_alignment_(start_alignment), triple_(std::move(triple)), code_stream_(code_) {
...
void CodeGenerator::AddInstruction(const llvm::MCInst &inst) {
  ...
  emitter_->encodeInstruction(inst, code_stream_, new_fixups, *sti_);//将inst写入code_stream，new_fixups保存了常量值
...
bool CodeGenerator::Initialize() {
  ...
  ctx_.reset(new llvm::MCContext(nullptr, mri_.get(), nullptr));
    emitter_.reset(target->createMCCodeEmitter(*mii_, *mri_, *ctx_));
...
Error CodeGenerator::LinkCode(uintptr_t location) {
  for (const ConstantPoolDataExpr *pool : const_pool_exprs_)
    const_cast<ConstantPoolDataExpr *>(pool)->setBaseLocation(location);//总共的长度是内存中函数地址+跳板大小


  for (const llvm::MCFixup &fixup : fixups_) {//arm和aarch64中仅保存了跳转的地址，也就是BR [addr]
