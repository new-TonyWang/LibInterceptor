...
class Target {
 public:
  ...
  virtual void *FixupCallbackFunction(void *old_function, void *new_function) {
    ...
    //跳过跳板函数
    virtual void SkipTrampolineFunction(void* &data,size_t offset,
                                        llvm::MCInst &inst)=0;
    //检测是否到达了函数结尾
    virtual bool EndOfFunction(const llvm::MCInst &inst)= 0;

    //检测是否是否被hook
    virtual bool CheckHook(const llvm::MCInst &inst)= 0;
    //重置状态
    void ResetState();
...
protected:
    ...
    enum state{
        HEALTHY,//正常情况
        DANGEROUS,//出现了跳转之前的寄存器分配
        DEAD,//确定该函数已经被hook
    };
    state currentState_;
    llvm::MCOperand jmpreg_;//保存跳转的寄存器
